...
def attempt_to_make_names_unique(entries_and_traces):
    ...
    for name in non_unique_names:
        ...
        first_trace_difference = next((i for i, trace_eles in enumerate(zipped_traces) if not all_the_same(trace_eles)),
                                      None)
...
def group_trace_by_operations(trace_df: pd.DataFrame) -> pd.DataFrame:
    ...
    trace_df.drop(attention_ops + quant_ops + sgmv_shrink_ops + sgmv_expand_ops + bgmv_shrink_ops + bgmv_expand_ops +
                  cutlass_gemm_ops + gemm_ops + rms_norm_ops + vocab_embed_ops + mem_ops + elementwise_ops +
                  nccl_all_reduce_ops + nccl_gather_ops + nccl_broadcast_ops + nccl_other_ops +
...
if __name__ == "__main__":
    ...
    main(Path(args.json_trace), output_directory, depth, args.plot_metric, make_names_unique, args.top_k,
         args.fold_json_node)
