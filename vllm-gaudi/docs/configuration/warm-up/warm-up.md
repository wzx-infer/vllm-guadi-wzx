...
The following example presents the same buckets as those described in the [Bucketing Mechanism](../../features/bucketing_mechanism.md) section. Each output line corresponds to the execution of a single bucket. When a bucket is executed for the first time, its graph is compiled and can be reused later, avoiding further graph compilations.
